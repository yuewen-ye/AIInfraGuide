...
rank = int(os.environ["RANK"])              # 当前进程在所有节点所有进程中的全局编号（0 ~ world_size-1）
local_rank = int(os.environ["LOCAL_RANK"])  # 当前进程在本机内的编号，通常直接用作 GPU 设备号
world_size = int(os.environ["WORLD_SIZE"])  # 参与训练的进程总数（等于总卡数，单进程单卡场景下）
```
...
import torch.distributed as dist
from torch.nn.parallel import DistributedDataParallel as DDP  # DDP：包裹模型后自动做梯度 AllReduce 同步
from torch.utils.data import DataLoader, TensorDataset
from torch.utils.data.distributed import DistributedSampler  # 按 rank 切分数据集，确保各卡数据不重叠
