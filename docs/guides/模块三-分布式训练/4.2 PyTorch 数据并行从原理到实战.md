...
import torch.distributed as dist
from torch.nn.parallel import DistributedDataParallel as DDP  # DDP：数据并行标准方案，自动完成初始参数广播和反向梯度 AllReduce
from torch.utils.data import DataLoader, DistributedSampler
from torchvision import datasets, transforms  # torchvision：PyTorch 官方视觉库，这里用它加载 MNIST 数据集
...
from torch.distributed.fsdp import (
    FullyShardedDataParallel as FSDP,  # FSDP：PyTorch 官方实现的参数分片方案，对应 DeepSpeed ZeRO 思想
    MixedPrecision,
    ShardingStrategy,
)
from torch.distributed.fsdp.wrap import size_based_auto_wrap_policy  # 按参数量阈值决定子模块是否单独分片
from torch.utils.data import DataLoader, DistributedSampler
from torchvision import datasets, transforms  # torchvision：PyTorch 官方视觉库，这里用它加载 MNIST 数据集
...
def main():
    ...
    bf16_policy = MixedPrecision(
        ...
        buffer_dtype=torch.bfloat16,  # buffer 指非可训练的模块状态，如 BatchNorm 的 running_mean/running_var
    )
    ...
    dataset = datasets.MNIST("./data", train=True, download=(rank == 0), transform=transform)  # 只让 rank 0 下载，避免多进程同时写文件冲突
...
```python
import torch.distributed.checkpoint as dcp  # DCP：支持分片保存/加载、且可跨卡数变更（resharding）的分布式 checkpoint 工具
