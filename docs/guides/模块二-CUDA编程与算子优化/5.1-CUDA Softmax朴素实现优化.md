...
__global__ void softmax_v1(float* input, float* output, int M, int N) {
    // extern __shared__ 声明一个大小在启动时才确定的共享内存数组，
    // 具体字节数由 Kernel 启动语法 <<<grid, block, smem_size>>> 的第三个参数指定
    ...
    smem[tid] = max_val;
    __syncthreads();  // 阻塞同步：确保所有线程都已把各自的局部最大值写入 smem，再统一读取

    // Shared Memory 规约求全局最大值：树形规约，每轮把参与规约的线程数减半（s 依次为 128、64...1），
    // tid < s 的线程负责合并 smem[tid] 与 smem[tid+s]，log2(blockDim.x) 轮后 smem[0] 即为全局结果
    for (int s = blockDim.x / 2; s > 0; s >>= 1) {
        if (tid < s) {
            smem[tid] = fmaxf(smem[tid], smem[tid + s]);
        }
        __syncthreads();  // 每轮规约后必须同步，避免下一轮读到还未写完的旧值
    }
    max_val = smem[0];
    __syncthreads();
...
// <<<grid, block, smem_size>>> 的第三个参数即动态共享内存大小（字节），
// 对应 Kernel 内 extern __shared__ 声明的数组实际占用的空间
...
```cuda
// Warp 内规约：Max
// 0xffffffff 表示 Warp 内全部 32 个线程都参与（掩码全 1）；offset 从 16 开始每轮减半，
// __shfl_down_sync(mask, val, offset) 让每个线程读到 lane id 为 (自己 + offset) 的线程的 val，
// 这样两两配对相加/取最大，log2(32)=5 轮后 lane 0 就持有整个 Warp 的规约结果
...
__device__ float blockReduceMaxShuffle(float val) {
    ...
    // 32 是 Warp 大小：lane 是线程在所属 Warp 内的编号（0~31），wid 是该线程属于第几个 Warp
    ...
    int num_warps = blockDim.x / 32;  // Block 内的 Warp 总数
    // 用前 num_warps 个线程各自持有一个 Warp 的局部最大值，其余线程填 -INFINITY 不影响结果
    val = (lane < num_warps) ? warp_max[lane] : -INFINITY;
    if (wid == 0) val = warpReduceMax(val);  // 第一个 Warp 对所有 Warp 的局部最大值做最终规约
...
__device__ float blockReduceSumShuffle(float val) {
    ...
    // 未被 Warp 局部结果占用的线程填 0.0f，加到求和里不影响最终结果
...
__global__ void softmax_v3(float* input, float* output, int M, int N) {
    ...
    // reinterpret_cast<float4*> 把 float 指针重新解释为 float4 指针，
    // 使一次内存操作能读写 4 个连续 float（16 字节），要求地址满足 16 字节对齐
    float4* x4 = reinterpret_cast<float4*>(x);
    float4* y4 = reinterpret_cast<float4*>(y);
    int N4 = N / 4;  // 整数除法向下取整，得到能被 float4 整除的部分；剩余元素在下方尾部循环单独处理
