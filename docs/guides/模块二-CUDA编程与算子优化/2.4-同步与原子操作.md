...
// threadIdx.x ^ 1 是按位异或：只翻转最低位，效果是让 0↔1、2↔3……两两配对交换，
// 从而让每个线程读到与自己相邻（成对）的线程写入的值
...
__global__ void multi_block_reduce(float* input, float* output,
    ...
    if (threadIdx.x == 0) {
        ...
        // atomicInc(addr, val) 语义特殊：返回旧值并将 *addr 加1，但若旧值 >= val 则重置为0，
        // 这里传入 gridDim.x 是为了让计数器在所有 Block 都递增一次后归零，便于下次复用
...
__global__ void hierarchical_reduce(float* input, float* output, int N) {
    ...
    // 第一层：Warp 内归约（无需原子操作）
    // 0xFFFFFFFF 表示 Warp 内 32 个线程全部参与 shuffle；offset 从 16 开始每次减半，
    // 每一步都让当前线程加上 lane id 更高、偏移 offset 的线程的值，最终归约到 lane 0
...
__device__ int atomicAggInc(int* counter) {
    // __activemask()：返回当前 Warp 中处于活跃状态（未被分支剔除）的线程掩码
    unsigned int active = __activemask();
    // __ffs(x)：返回 x 中最低位的 1 是第几位（从1开始计数），故减1得到该线程在 Warp 内的 lane id
    int leader = __ffs(active) - 1;  // 选出领导者（最低位的活跃线程）
    int lane = threadIdx.x % 32;

    // __popc(x)：统计 x 的二进制表示中有多少个 1（population count）
    ...
    // 用活跃线程掩码 active 做 shuffle，把 leader 持有的 old_val 广播给其他活跃线程
    old_val = __shfl_sync(active, old_val, leader);

    // 每个线程得到自己的唯一递增值
    // (1 << lane) - 1 生成一个低 lane 位全为 1 的掩码，与 active 相与后数 1 的个数，
    // 即得到"比自己 lane id 小的活跃线程数"，作为自己在这批递增值中的排名
...
```cpp
// cooperative_groups 是 CUDA 9+ 提供的线程分组库，把 Block/Warp/Tile/Grid
// 等不同粒度的线程集合抽象成统一的 group 对象，支持灵活的同步与协作操作
...
// cudaLaunchCooperativeKernel 是专用启动 API：保证所有请求的 Block 会被同时调度上 GPU，
// 这是 grid.sync() 能安全工作的前提（普通 <<<>>> 启动不保证这一点）
