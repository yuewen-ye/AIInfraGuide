...
// 除以 32 是因为一个 Warp 固定包含 32 个线程，同一 Warp 内的线程算出的商相同，
// 因此会走向同一分支，避免了 Warp 内的分歧
...
```cpp
// mask：参与操作的 lane 掩码（通常填 0xffffffff 表示全部 32 个 lane 都参与）；width：逻辑上把 Warp 切成多少个一组（默认 32，即整个 Warp）
T __shfl_sync(unsigned mask, T var, int srcLane, int width=32);        // 从 srcLane 广播 var 的值给组内所有线程
T __shfl_up_sync(unsigned mask, T var, unsigned delta, int width=32);  // 读取比自己 lane id 小 delta 的线程的 var
T __shfl_down_sync(unsigned mask, T var, unsigned delta, int width=32);// 读取比自己 lane id 大 delta 的线程的 var，常用于规约
T __shfl_xor_sync(unsigned mask, T var, int laneMask, int width=32);   // 与 (laneId ^ laneMask) 对应的线程交换 var，常用于蝶形规约
```
