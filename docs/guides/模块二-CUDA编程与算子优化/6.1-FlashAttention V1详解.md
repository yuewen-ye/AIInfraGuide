...
import math
import torch
import triton               # Triton：用类 Python 语法编写、自动编译为高效 GPU Kernel 的 DSL
import triton.language as tl  # tl 提供 Kernel 内可用的向量化操作（load/store/dot/max 等）

@triton.jit  # 标记该函数会被 Triton 的 JIT 编译器编译为 GPU Kernel
def flash_attn_v1_kernel(
    ...
    # tl.program_id(axis) 返回当前 program 在 grid 该维度上的编号，类似 CUDA 的 blockIdx
    ...
    # tl.arange(0, BLOCK_D) 生成 [0, BLOCK_D) 的向量，用作后续按元素索引取数据的偏移量
    ...
    for kv_start in range(0, N, BLOCK_N):
        ...
        # tl.load(ptr, mask, other) 按给定地址批量读取数据；mask 为 False 的位置不读取，用 other 填充，
        # 这里的 [:, None] / [None, :] 是 Triton 的广播写法，用来拼出二维（行, 列）访问地址
        ...
        for q_start in range(0, N, BLOCK_M):
            ...
            # tl.dot 是矩阵乘法（底层映射到 Tensor Core），tl.trans 做转置
...
def flash_attn_v1(Q, K, V, BLOCK_M=64, BLOCK_N=64):
    ...
    # triton.next_power_of_2 向上取整到最近的 2 的幂：tl.arange 等 Triton 向量化操作要求长度为 2 的幂，
    # 当 D 本身不是 2 的幂时，BLOCK_D > D，多出来的部分靠上面的 mask（d_range < D）屏蔽掉
