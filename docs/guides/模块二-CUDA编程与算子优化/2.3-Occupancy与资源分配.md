...
// 两个参数分别约束"每 Block 最大线程数"和"每 SM 至少驻留的 Block 数"，
// 编译器据此反推每线程寄存器上限，从而在编译期而非运行期控制 occupancy
...
int main() {
    ...
    // cudaOccupancyMaxPotentialBlockSize：CUDA Runtime 提供的辅助函数，
    // 根据 kernel 的资源占用自动推荐能最大化 Occupancy 的 Block 大小
    ...
    // cudaOccupancyMaxActiveBlocksPerMultiprocessor：查询给定 Block 大小和
    // 共享内存用量下，一个 SM 最多能同时驻留多少个 Block
    int maxActiveBlocks;
...
```bash
# -Xptxas -v 将 -v 参数转发给 ptxas（PTX 汇编器），输出每个 Kernel 的资源使用
