...
__global__ void reduce_v0(float* input, float* output, int n) {
    ...
    for (int step = 1; step < blockDim.x; step *= 2) {
        // 每隔 2*step 个线程才有一个参与计算，同一 Warp 内会出现有的线程满足、
        // 有的不满足条件，导致 Warp Divergence（下文详述）
        if (tid % (2 * step) == 0) {
...
```cuda
// BLOCK_SIZE 是模板的非类型参数（编译期常量），必须在调用点用 switch 枚举出所有支持的取值，
// 这样编译器才能为每个 BLOCK_SIZE 生成一份特化代码（循环展开、边界判断都在编译期确定）
switch (block_size) {
    // 第三个尖括号参数是动态共享内存大小：每个线程占 1 个 float，因此等于 block_size * sizeof(float)
...
__global__ void reduce_v7(float* input, float* output, int n) {
    ...
    // reinterpret_cast 将 float* 重新解释为 float4*，使一条指令能读取 16 字节（4 个 float），
    // 要求 input 地址按 16 字节对齐，否则访问会出错
