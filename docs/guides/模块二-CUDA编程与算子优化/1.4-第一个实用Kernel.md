...
int main() {
    ...
    // 先加上 (blockSize - 1) 再做除法，确保哪怕有余数也能多分一个 Block 覆盖剩余元素
...
```cpp
// do { ... } while (0) 是宏定义的经典写法：把多条语句包裹成一个整体，
// 这样宏在 if-else 等语句中展开后仍然是单条语句，末尾加分号也不会出错
...
```cpp
cudaEventRecord(start);  // 把计时起点插入到当前 Stream，覆盖下面全部操作（而非只有 Kernel）
...
cudaEventRecord(stop);
cudaEventSynchronize(stop);  // 阻塞 CPU 直到 stop 事件被 GPU 实际执行到，确保下面读到的是完整耗时

float totalTime = 0;
cudaEventElapsedTime(&totalTime, start, stop);
// 这里包含了两次 H2D + 一次 D2H 拷贝的时间，所以比纯 Kernel 计时（见上一节）大得多，
// 真实吞吐评估应该看这个含传输的端到端数字，而不是只看 Kernel 本身
...
int main() {
    ...
    int blockSize = 256;
    // 向上取整技巧：保证元素数不能整除 blockSize 时也能多分配一个 Block
    int gridSize = (N + blockSize - 1) / blockSize;
    ...
    // 3.0f * size 对应 2 次读 + 1 次写共 3 倍数组大小；1e-3f 把 ms 转成 s，1e9f 把 Byte 转成 GB
...
__global__ void vectorAddStride(const float* A, const float* B,
                                float* C, int N) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    // stride = 本次启动的总线程数（Grid 内所有 Block 的线程数之和）
    // Grid-stride loop：每个线程先处理 idx，再跳过 stride 个位置处理下一个元素，
    // 这样固定数量的线程也能覆盖任意大小的 N，不用按 N 反推 Grid 大小
...
```cpp
// CUDA Stream：GPU 上的一条命令队列，同一 Stream 内的操作按顺序执行，
// 不同 Stream 之间的操作可以并发/重叠，是实现异步传输和计算重叠的基础
cudaStream_t stream;
cudaStreamCreate(&stream);

// cudaMemcpyAsync 是异步调用：发起后立刻返回，不会阻塞 CPU 等待传输完成
// 异步传输和计算可以重叠
cudaMemcpyAsync(d_A, h_A, size, cudaMemcpyHostToDevice, stream);
cudaMemcpyAsync(d_B, h_B, size, cudaMemcpyHostToDevice, stream);
// 第三个参数 0 表示不额外申请动态共享内存，第四个参数指定该 Kernel 提交到哪个 Stream
vectorAdd<<<gridSize, blockSize, 0, stream>>>(d_A, d_B, d_C, N);
cudaMemcpyAsync(h_C, d_C, size, cudaMemcpyDeviceToHost, stream);

// 阻塞等待该 Stream 内所有操作完成，之后才能安全读取 h_C
