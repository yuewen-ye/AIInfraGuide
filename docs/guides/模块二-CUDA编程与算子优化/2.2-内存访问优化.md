...
// ✅ 行方向遍历：连续线程（col 递增）访问同一行的连续列，地址相差 1 个元素，合并访问
int col = threadIdx.x + blockIdx.x * blockDim.x;
int row = blockIdx.y;
float val = matrix[row * cols + col];  // 合并

// ❌ 列方向遍历：连续线程（row2 递增）访问同一列的连续行，地址相差 cols 个元素
...
// tile 是 32x32 的共享内存数组，行宽正好是 32（Bank 数量），后面读取时会因此产生冲突
...
__global__ void safe_vectorized(float* data, int offset, int N) {
    // 处理头部未对齐的元素
    // (4 - offset % 4) % 4 计算凑齐 16B（4 个 float）对齐所需跳过的元素个数：
    // 外层 %4 是防止 offset 本身已经 4 对齐时算出 4（应为 0）
...
// (width + TILE_DIM - 1) / TILE_DIM 是向上取整技巧，确保 width/height 不能整除 TILE_DIM 时也能覆盖全部数据
...
__global__ void reduce_interleaved(float* input, float* output, int N) {
    ...
    // tid % (2*s) == 0 使参与计算的线程在 Warp 内变得稀疏、地址跳跃，
    // 既造成线程分支（Warp 内一部分线程空闲）也容易引发 Bank Conflict
...
```cpp
// __ldg() 是 CUDA 提供的只读加载内建函数，通过只读数据缓存（texture cache）加载
// 适合只读数据，可避免 L1 缓存污染
