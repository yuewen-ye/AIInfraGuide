...
__global__ void sgemm_naive(const float* A, const float* B, float* C,
                            int M, int N, int K) {
    // 每个线程负责 C 中一个元素，行列坐标由 Block/Thread 索引换算得到
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;

    // 边界检查：当 M、N 不能被 blockDim 整除时，避免越界线程写非法地址
    if (row < M && col < N) {
        float sum = 0.0f;
        for (int k = 0; k < K; k++) {
            // A 按行读取（连续），B 按列读取（每次跨 N 个元素，非连续）
...
// 加载 tileA 时的线程坐标（8列×32行）：把 256 个线程重排成 32行×8列的网格
int a_thread_x = tid % 8;       // 列方向
int a_thread_y = tid / 8;       // 行方向

// 加载 tileB 时的线程坐标（32列×8行）：把 256 个线程重排成 8行×32列的网格
...
// 每个线程负责 Tm×Tn 个输出元素，Tm/Tn 即线程网格在 M/N 方向的"跳跃"次数
...
__global__ void sgemm_block_tiling(float* A, float* B, float* C,
    ...
    int r0 = blockIdx.y * BM;  // 当前 Block 负责的 tileC 在矩阵 C 中的起始行
    int c0 = blockIdx.x * BN;  // 当前 Block 负责的 tileC 在矩阵 C 中的起始列
    int tid = threadIdx.x;
    ...
    for (int k = 0; k < K; k += BK) {
        ...
        for (int i = a_thread_y; i < BM; i += A_BLOCK_Y) {
            ...
            // 越界位置填 0（zero-padding），保证 M/K 非整除 BM/BK 时结果仍正确
        ...
        for (int j = b_thread_x; j < BN; j += B_BLOCK_X) {
            ...
            // 同上，K/N 非整除 BK/BN 时越界部分填 0
            Bs[b_thread_y][j] = (r < K && c < N) ? B[r * N + c] : 0.0f;
        }

        __syncthreads();  // 等待 Block 内所有线程完成加载，才能开始读取 As/Bs 计算

        // 外积方式计算 As × Bs
        #pragma unroll
        for (int p = 0; p < BK; p++) {
            for (int i = 0; i < Tm; i++) {
                // 按 C_BLOCK_Y 跨步取值，对应该线程负责的离散输出行位置
        ...
        __syncthreads();  // 等待所有线程用完当前 As/Bs，才能进入下一轮迭代覆盖它们
    }
...
__global__ void sgemm_thread_tiling(const float* A, const float* B, float* C,
    ...
    // 每个线程在 C 中负责 TM×TN 的子块（此实现中子块是连续的，而非 4.5 节中的跨步分布）
    ...
    for (int bk = 0; bk < K; bk += BK) {
        ...
        __syncthreads();  // 等待加载完成才能读取 As/Bs

        // 外积累加
        for (int k = 0; k < BK; k++) {
            // 从 Shared Memory 加载到寄存器：显式分离"读 smem"与"计算"两步
            for (int i = 0; i < TM; i++)
                a_frag[i] = As[thread_row + i][k];
            for (int j = 0; j < TN; j++)
                b_frag[j] = Bs[k][thread_col + j];

            // 寄存器上做外积：TM+TN 次 smem 读取驱动 TM×TN 次 FMA
            for (int i = 0; i < TM; i++)
                for (int j = 0; j < TN; j++)
                    c_frag[i][j] += a_frag[i] * b_frag[j];
        }
        __syncthreads();  // 等待所有线程算完，才能进入下一轮覆盖 As/Bs
    }
...
```cuda
// >> 5 等价于 / 32，& 31 等价于 % 32（32 是 2 的幂，位运算比除法/取模更快）
...
```cuda
// 将 float 指针强转为 float4 指针再取值，一次读写 4 个连续 float（128 bit）
...
// 转置：Global 中连续的 4 个元素，分散写到 As 的同一列、不同行（As[BK][BM] 布局）
...
// 从 Shared Memory 读取时可以用 float4（转置后地址连续）
...
```cuda
for (int i = 0; i < Tm; i++) {
    // i/4 选组（第几组 4 行），i%4 选组内偏移（组内 4 行是连续的）
...
```cuda
// 每行多分配 4 个 float 的 padding：让下一行的起始 bank 错位，避免固定跨行访问落在同一 bank
...
```cuda
// 写入时 swizzle：用 k 的函数对列号做异或扰动，使不同 k 层的同一逻辑列落在不同 bank
As[k][row ^ (k * 4)] = value;

// 读取时用相同规则反算地址（异或是自逆操作，用同样的 xor 值即可还原真实地址）
...
// Z-order 映射：不是简单的行主序 lane_id / 8，而是让 xor 1 / xor 2 分别只翻转 col / row
...
```cuda
float regA[2][Tm], regB[2][Tn];  // 两组缓冲区，一组用于计算，另一组同时被写入下一步数据

#pragma unroll
// 循环多跑一次（BK+1）：让最后一步只做计算、不再预取，形成经典的软件流水线（software pipelining）
for (int p = 0; p < BK + 1; p++) {
    // 计算阶段：使用上一步（p-1）已经读取好的数据，而不是本步刚读的
    if (p > 0) {
        for (int i = 0; i < Tm; i++)
            for (int j = 0; j < Tn; j++)
                // (p - 1) & 1 即 (p-1) % 2，在两个缓冲区间切换
                Ct[i][j] += regA[(p - 1) & 1][i] * regB[(p - 1) & 1][j];
    }

    // 预取阶段：读取本步数据到另一组寄存器（与上面的计算阶段可被硬件并行调度）
    if (p < BK) {
...
```cuda
// 最外层维度 2 表示双缓冲的两个 buffer，一个用于当前计算，另一个用于加载下一个 K-tile
...
__global__ void sgemm_double_buffer(const float* A, const float* B, float* C,
    ...
    float c_frag[TM][TN] = {0.0f};
    float ldg_a[4], ldg_b[4];  // 暂存 Global Memory 加载结果，尚未写入 Shared Memory

    int buf_idx = 0;  // 当前正在使用（计算）的 Shared Memory buffer 编号

    // Prologue: 加载第一个 tile，为主循环准备好初始数据
    ...
    for (int tile_k = BK; tile_k < K + BK; tile_k += BK) {
        int next_buf = 1 - buf_idx;  // 另一块 buffer，用于本轮加载下一个 K-tile

        #pragma unroll
        for (int k = 0; k < BK; k++) {
            // 预取下一个 k 步的寄存器数据（与下面本步的计算并行执行）
            if (k + 1 < BK) {
                load_shared_to_reg(As[buf_idx], Bs[buf_idx],
                                   a_frag[1 - k % 2], b_frag[1 - k % 2], k + 1);
            }

            // 在第一个 k 步时发射下一 tile 的 Global Memory 加载（LDG 是异步的，
            // 发出后线程立即继续往下执行，无需等待数据到达）
            if (k == 0 && tile_k < K) {
        ...
        // 将 LDG 结果写入下一 buffer（此时之前发出的异步加载应已完成）
        if (tile_k < K) {
            store_reg_to_shared(ldg_a, ldg_b, As[next_buf], Bs[next_buf]);
        }
        __syncthreads();  // 整个 Block 都写完 next_buf 后才能切换过去使用
...
```
// 异步地将 16 字节从 Global Memory 直接搬运到 Shared Memory，无需经过寄存器
...
```cuda
// 提交一组异步拷贝（之前发出的 cp.async 指令归为一组）
cp.async.commit_group;

// 阻塞等待，直到未完成的异步拷贝组数量 <= N（N=0 即等待全部完成）
