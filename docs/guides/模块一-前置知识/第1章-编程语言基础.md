...
🌱 **新手视角**：如果你只写过 Python，可能会好奇“分发器”是什么、为什么中间要经过这么多层。简单说：`torch.matmul` 这个 Python 函数本身不做任何数学计算，它只是一个“转发请求”的接口——根据你传入张量的设备（CPU/GPU）和数据类型，找到真正该调用的底层实现（比如在 GPU 上转发给 cuBLAS 库或某个专门写的 CUDA Kernel），再由这些用 C++/CUDA 写的代码去操作硬件完成计算。这一层“转发”逻辑就是“算子分发器”（dispatcher）。你现在不需要记住它的实现细节，只需要建立一个印象：**你调用的 Python 函数名，和真正执行计算的代码，往往不是同一段代码**，中间可能隔着好几层。
...
🌱 **新手视角**：习惯了 C/Java 的人常把变量想象成“一个装值的盒子”，赋值 `b = a` 就是把 a 盒子里的内容复制一份放进 b 盒子。Python 完全不是这样：对象都存在内存里的某个地方，变量名只是贴在对象上的“标签”。`a = [1, 2]` 的意思是“把标签 a 贴在这个新建的列表对象上”；`b = a` 是“把标签 b 也贴在同一个对象上”——两张标签指向同一个东西，不是两个东西。所以 `b.append(3)` 改的是那唯一的对象，`a` 当然也会看到变化。
...
🌱 **新手视角**：`__len__`、`__iter__` 这种前后各两个下划线的方法名，社区习惯称为“dunder 方法”（double underscore 的缩写），也叫魔术方法（magic method）。它们不是让你直接手动调用的，而是 Python 语言本身在特定语法背后偷偷调用的钩子：你写 `len(obj)`，Python 内部实际执行的是 `obj.__len__()`；你写 `for x in obj`，Python 内部会去找 `obj.__iter__()`。只要按约定实现了这几个方法，你自定义的类就能“无缝接入”Python 的内置语法，而不需要修改语言本身。
...
class Batch:
    ...
    def __len__(self) -> int:  # 实现后 len(batch) 才能工作，而不是只能用 len(batch.samples)
        return len(self.samples)

    def __iter__(self) -> Iterator[list[float]]:  # 实现后 for sample in batch 才能工作
...
class Device:
    ...
    def parse(cls, value: str) -> "Device":
        ...
        return cls(kind=kind, index=int(index))  # cls 而非 Device：子类调用 parse 时会正确构造子类实例
...
🌱 **新手视角**：区分“可迭代对象”和“迭代器”最直观的类比是——一本书是“可迭代对象”，你可以从它“生成”一个书签（迭代器）；书签本身记录着“读到哪一页了”，每翻一页（调用一次 `__next__`）书签的位置就往前移动一格，读完最后一页会报“没有更多内容”（`StopIteration`）。同一本书可以同时生成多个独立书签（多个迭代器互不影响），但同一个书签只能往前翻，不能重来。
    ...
    with open(path, encoding="utf-8") as file:
        for line in file:
            ...
            if len(batch) == batch_size:
                yield batch      # 攒够一个 batch 就产出，调用方消费后再回来继续读文件
                batch = []       # 必须建一个新列表，不能 clear()：外部可能还持有刚 yield 出去的那个 batch 引用
    if batch:
        yield batch  # 文件末尾如果剩下不满一个 batch 的数据，也要产出，否则会被静默丢弃
```
...
🌱 **新手视角**：可以把装饰器想象成给函数“套一层外壳”。加了 `@timed` 之后，外部代码看起来还是在调用 `preprocess(...)`，但实际执行的是 `wrapper(...)`——它会先做一些准备工作（记录开始时间），中间“真正”调用一次原始的 `preprocess`，最后再做收尾工作（计算并打印耗时）。原函数被“包”在外壳里面，调用者通常感觉不到这层包装存在，这也是它叫“装饰器”的原因。
...
def parse_world_size(raw: str) -> int:
    try:
        ...
        # from exc：保留原始异常作为 __cause__，traceback 里能同时看到"根因"和"转换后的异常"
...
🌱 **新手视角**：GIL 的存在不是“故意为难开发者”，而是历史选择。CPython 内部有大量共享数据结构（比如每个对象的引用计数），如果不加保护，多个线程同时读写这些计数就会产生数据错乱。给每个对象都加一把细粒度的锁开销太大且容易死锁，CPython 干脆用一把“全局”的锁作为简化方案——效果就是，任意时刻只有一个线程能真正执行 Python 字节码，其他线程即使处于“运行”状态，也可能只是在排队等这把锁。
...
def increment() -> None:
    global counter  # 声明要重新绑定模块级的 counter，而不是创建一个同名局部变量
    with lock:      # 临界区只包住真正的读改写，缩小锁的持有时间
        counter += 1
```
...
def benchmark(fn, warmup: int = 5, repeat: int = 20) -> float:
    for _ in range(warmup):  # 预热：跳过首次调用的一次性开销（JIT/缓存/懒加载等），不计入计时
        fn()

    samples = []
    for _ in range(repeat):  # 多次采样而非只跑一次，抵消系统噪声（调度、GC、频率变化等）
        start = perf_counter()
        fn()
        samples.append(perf_counter() - start)
    return median(samples)  # 用中位数而非平均值：对偶发的极端慢值（如一次 GC 停顿）更不敏感
```
...
🌱 **新手视角**：在 Python 里，你几乎不需要关心内存什么时候释放——对象没有任何引用指向它时，Python 的垃圾回收器（Garbage Collector, GC）会自动帮你清理。C/C++ 默认不提供这种自动回收：你申请的内存，需要由你（或某个你写的机制）明确决定“什么时候释放”。这一节要讲的，正是 C++ 世界里“如何在没有自动 GC 的情况下，依然可靠地管理内存”这件事。

### 4.1 值、地址、指针与引用

🌱 **新手视角**：程序运行时，所有变量的值都存放在内存的某个位置，每个位置都有唯一的编号，这个编号就是“地址”。Python 帮你隐藏了地址这件事——你只管用变量名，Python 在背后自动管理对象存在哪、什么时候回收。C/C++ 把这一层暴露给你：“指针”就是一个专门用来存“地址”的变量，可以把它想象成一张写着门牌号的纸条；“解引用”（`*ptr`）就是“按纸条上的门牌号去敲门”，找到真正的房间（内存位置）并读写里面的值。
...
🌱 **新手视角**：你可能听过“栈”（stack）和“堆”（heap）这两个词——粗略理解：函数内声明的普通局部变量通常放在“栈”上，函数一返回，这些空间就自动回收，很快很省心；用 `new`/`malloc` 显式申请的内存放在“堆”上，容量更大更灵活，但不会自动回收，必须自己记得释放。
...
class File {
public:
    explicit File(const char* path)  // explicit：禁止 File f = "path" 这种隐式转换构造
    ...
    ~File() {
        std::fclose(handle_);  // 析构时统一释放，构造失败已 throw，走不到这里，不会用空指针调用 fclose
    }

    // 删除拷贝：FILE* 只能被关闭一次，若允许拷贝，两个 File 对象会持有同一个 handle_，
    // 其中一个析构后另一个的 handle_ 就变成悬空指针
...
🌱 **新手视角**：可以把这个过程类比成“组装家具”：预处理是把说明书里引用的其他小册子（`#include`）先摊开拼进来；编译是把文字说明翻译成“零件加工图”（汇编/目标文件），这一步只检查单个文件自己是否合理，还不管其他文件；链接则是把所有零件的加工图拼在一起，把“这个函数具体在哪”这类互相引用的问题全部对上号，最终拼出一个完整能跑的程序。如果某个零件的加工图里引用了别处没提供的东西，就会出现“链接错误”（`undefined reference`）。
...
🌱 **新手视角**：如果你熟悉 Python 中“函数不区分参数类型，什么类型都能传”，C++ 模板做的是完全不同的事：它不是运行时才决定怎么处理不同类型，而是在**编译期**针对你实际用到的每一种类型，“照着模板”生成一份专门的代码。写一次 `add<T>`，编译器看到你调用 `add(1, 2)` 和 `add(1.0f, 2.0f)`，就会分别生成两份不同的机器码（`add<int>` 和 `add<float>`）——这也是为什么模板出错时，报错信息经常牵扯到“实例化”这个词。
...
🌱 **新手视角**：Python 解释器和 C++ 编译出的机器码，本质上是两套互不认识对方内部细节的程序。“跨语言边界”要解决的问题就是：Python 怎么把一段数据的地址、类型信息传给 C++，C++ 算完之后又怎么把结果安全地交回 Python，同时不破坏任何一方对内存的假设。下面的 `ctypes` 和 `pybind11` 就是两种不同复杂度的“翻译协议”。
...
🌱 **新手视角**：这一节涉及大量命令，不需要死记硬背。更好的方式是先明白：“命令行（Shell）本质上是你和操作系统对话的文本界面”——你敲一行命令并回车，Shell 把它解析后交给对应的程序执行，执行结果再打印回屏幕。下面的命令建议边看边在自己的终端里敲一遍，观察真实输出，比单纯阅读更容易建立直觉。
...
export PATH="$CUDA_HOME/bin:$PATH"                                  # 让 nvcc 等工具可以直接在命令行调用
export LD_LIBRARY_PATH="$CUDA_HOME/lib64:${LD_LIBRARY_PATH:-}"       # 让运行时能找到 CUDA 动态库；:-} 防止变量未设置时报错（配合 set -u）
...
🌱 **新手视角**：为什么需要“隔离环境”？因为不同项目常常依赖不同版本的 Python、不同版本的库（甚至互相冲突的版本），如果所有项目共用同一套全局环境，装好项目 A 很容易弄坏项目 B。“环境隔离”要解决的核心问题就是：让每个项目都拥有一套“只属于自己”的依赖集合，互不干扰。
...
🌱 **新手视角**：如果你对 Git 还没有直觉，先记住三个词：仓库（repository，被 Git 追踪修改历史的项目目录）、提交（commit，给当前所有文件状态拍的一张“快照”，并记录相对上一次快照改了什么）、分支（branch，一条独立的提交历史线，方便你在不影响主线的情况下尝试改动）。下面的命令都是在这三个概念上操作。
...
🌱 **新手视角**：这一节把前面第 4、5、6 节学到的东西串成一条完整链路——用 C++ 写算子实现（第 4 节的内存管理），用 CMake 把它编译链接成动态库（第 5 节的编译链接），再用 `pybind11` 把它绑定成 Python 能直接 `import` 的模块（第 6 节的跨语言互操作）。跟着敲一遍代码，能帮你把前面分散学到的概念真正连起来。
