...
```python
# QK^T 得到注意力分数矩阵；除以 sqrt(d_k) 缩放，防止 d_k 较大时点积数值过大导致 softmax 梯度消失
scores = torch.matmul(Q, K.transpose(-2, -1)) / math.sqrt(d_k)
# 因果掩码给未来位置加上 -inf，softmax 后这些位置的权重会变成 0
...
def apply_rope(x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor):
    ...
    # 将 x 的最后一维拆成前后两半（LLaMA 等实现常见的配对方式），
    # 而非原始 RoPE 公式里相邻的 (2i, 2i+1) 配对，两者数学上等价，只是配对顺序不同
    x1 = x[..., : x.shape[-1] // 2]
    x2 = x[..., x.shape[-1] // 2 :]
...
class MaskedMultiHeadAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, rope_cos: torch.Tensor,
        ...
        # unsqueeze(0).unsqueeze(0) 把 (seq, seq) 扩展为 (1, 1, seq, seq)，
        # 依靠广播机制加到形状为 (batch, heads, seq, seq) 的 scores 上
