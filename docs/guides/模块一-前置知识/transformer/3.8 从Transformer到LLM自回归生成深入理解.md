...
def autoregressive_generate(model, prompt_tokens, max_new_tokens, temperature=1.0):
    ...
    for _ in range(max_new_tokens):
        ...
        # torch.multinomial 按给定的概率分布做随机采样，返回采样到的索引（这里即 token ID）
...
```python
# torch.topk 返回概率最大的 k 个值及其对应的索引（token ID）
...
# 找到累积概率刚好超过 p 的位置
# nonzero(as_tuple=True)[0] 取出所有满足条件的位置索引，取第一个即为首次超过 p 的位置
...
# 追加到缓存
# dim=2 对应张量形状 (batch, num_kv_heads, seq_len, head_dim) 中的 seq_len 维度
