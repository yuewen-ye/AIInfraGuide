...
def get_len_mask(b: int, max_len: int, feat_lens: torch.Tensor, device: torch.device) -> torch.Tensor:
    ...
    for i in range(b):
        # 先全部标记为屏蔽（1），再把每个样本真实长度内的列位置置 0（不屏蔽）
...
def get_subsequent_mask(b: int, max_len: int, device: torch.device) -> torch.Tensor:
    ...
    """
    # diagonal=1：只保留严格上三角（不含主对角线），即 j > i 的位置——这些正是第 i 个位置看不到的“未来” token
...
def get_enc_dec_mask(
    ...
    for i in range(b):
        # 每个样本的真实长度之后（feat_lens[i]: 到末尾）都是 padding，标记为屏蔽（1）
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        if attn_mask is not None:
            # masked_fill_ 带下划线表示原地（in-place）操作，直接修改 scores 本身
        ...
        # transpose 后张量在内存中不再连续，reshape 前必须先 contiguous() 拷贝为连续内存
...
def pos_sinusoid_embedding(seq_len: int, d_model: int) -> torch.Tensor:
    ...
    for i in range(d_model):
        # 偶数维用 sin、奇数维用 cos；i // 2 还原出公式中的下标 i（第几组 sin/cos 对）
